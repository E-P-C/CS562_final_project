...
def query():
            ...
            if eval(predicate):
                for h_row in _global:
                    if all(h_row[g] == row[g] for g in ['cust']):
                        for agg in ['sum_1_quant', 'count_1']:
    ...
    filtered_rows = []
    for row in _global:
        filtered = {k: row[k] for k in ['cust', 'sum_1_quant', 'count_1'] if k in row}
        print(filtered)
        filtered_rows.append(filtered)

    # Save to output.csv
    with open(f"MF1_output.csv", "w", newline="") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=['cust', 'sum_1_quant', 'count_1'])
        writer.writeheader()
        writer.writerows(filtered_rows)
    print(f"Output saved to MF1_output.csv")
